...
class RelativePermeability(WebvizPluginABC):
    def __init__(self, path_to_population_data_csv_file: Path ) -> None:
        ...
        try:
            self.population_df = pd.read_csv(path_to_population_data_csv_file) # df = data frame
            ...
            self.error_message = (
                f"The file {path_to_population_data_csv_file}' not found."
                "Please check you path"
            )
            return
        except pd.errors.ParserError:
            self.error_message = (
                f"The file '{path_to_population_data_csv_file}' is not a valid csv file."
            )
        except pd.errors.EmptyDataError:
            self.error_message = (
                f"The file '{path_to_population_data_csv_file}' is an empty file."
            )
        except Exception:
            self.error_message = (
                f"Unknown exception when trying to read '{path_to_population_data_csv_file}"
            )